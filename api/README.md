...
`test-server`(NestJS)에 구현된 엔드포인트를 기준으로 작성되었습니다.

> ⚠️ **2026-08-13 정정**: 실제 프로덕션 백엔드는 `server/`(Express, 승훈 개발)입니다. `test-server`는 인호가 API 명세 검증용으로 만든 별도 구현체이며, 실제 배포 대상이 아닙니다. 이 문서의 엔드포인트/응답 형태는 설계 기준으로는 유효하지만, `server/`의 실제 동작(특히 파일 업로드 방식)과 다를 수 있으니 코드 기준으로 다시 확인이 필요합니다.
...
## 파일 저장 방식 (2026-08-13 변경)

- 업로드 파일(사진, PDF, 오디오 등)은 디스크나 외부 스토리지(GCS)가 아니라 **DB의 공통 `files` 테이블에 바이너리로 저장**합니다.
- 각 도메인의 `photo_url` / `file_url` / `thumbnail_url` / `poster_url` 필드는 그대로 존재하지만, 값이 `/files/{id}` 형태입니다. 예전 문서의 `https://storage.googleapis.com/...` 예시나 `/uploads/...` 예시는 과거 설계/구버전 기준이니 참고만 하세요.
- 파일 바이너리를 내려주는 `GET /files/:id` 엔드포인트는 **아직 구현되지 않았습니다.** (`files` 테이블과 마이그레이션된 데이터는 있으나, 서빙용 컨트롤러가 없는 상태 — [docs/ORACLE_DEPLOYMENT.md](../docs/ORACLE_DEPLOYMENT.md) 참고)
- 업로드(POST) 엔드포인트들도 아직 디스크 저장 코드 기준으로 남아있어 실제로는 `files` 테이블에 쓰지 않습니다. 업로드/서빙 로직 교체가 백엔드 작업으로 남아있습니다.
