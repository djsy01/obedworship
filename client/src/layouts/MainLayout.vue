...
const loadAssets = async () => {
    try {
        ...
        try {
            ...
            if (songsRes.data && Array.isArray(songsRes.data) && songsRes.data.length > 0) {
...
<template>
    <div class="app-root">
                ...
                <div class="auth-area desktop-auth">
                    ...
                    <div v-else class="user-menu-wrapper">
                        ...
                        <div v-if="showDropdown" class="dropdown-menu">
                            <button @click="goToMyPage" class="dropdown-item">👤 마이페이지</button>
                            <button v-if="isAdmin" @click="goToAdmin" class="dropdown-item admin">⚙️ 관리자</button>
        ...
        <transition name="mobile-menu">
            <div v-if="showMobileMenu" class="mobile-menu-overlay" @click="closeMobileMenu" @touchmove.prevent>
            ...
            -->
                        ...
                        <RouterLink v-if="!isLoggedIn" to="/login" class="mobile-nav-link login-link" @click="closeMobileMenu"> 로그인 </RouterLink>

                        <template v-else>
                            <div class="mobile-user-info">
                                {{ userName }}
                            </div>
                            <button @click="goToMyPage" class="mobile-nav-link">👤 마이페이지</button>
                            <button @click="handleLogout" class="mobile-nav-link logout-link">🚪 로그아웃</button>
                        </template>
                    </div>
                </nav>
            </div>
        ...
        <footer class="app-footer">
            <div class="footer-content">
                <p class="footer-text">© {{ new Date().getFullYear() }} OBED Worship. All rights reserved.</p>
                <div class="footer-social">
                    ...
                    >
                        <img :src="soundcloudIcon" alt="Soundcloud" class="social-icon-footer" />
                    </a>
                    <a
